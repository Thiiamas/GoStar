...
func aStarVariation(start , goal *utils.Grid) []*utils.Grid{
	...
	for len(openSet) != 0 {
		...
		for _, neigh := range neighbors {
			...
			in2,_ := utils.Contains(closed, neigh);
			if !(in && in2) {
				cameFrom[neigh] = currentBoard
				// g(s) = g(e) + C(e-->)
				gScore[neigh] = gScore[currentBoard] + 1
			} else {
				if gScore(neigh) + distance(neigh, goal) > gScore[currentBoard] + 1 + distance(neigh, goal) {
					gScore[neigh] = gScore[currentBoard] + 1
					cameFrom[neight] = currentBoard
					if in := utils.Contains(closed, neight); in{
