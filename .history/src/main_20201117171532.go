...
func main() {
	size := 3
	board := createBoard(size,"test")
	goal := findGoal(size)

	board.Draw()
	fmt.Println("//////////////////")
	modified := board.Values
	modified[board.GetIndex(0)] = len(modified) +10
	sorted, count := quickSort(board.Values, 0, len(board.Values) - 1)
	fmt.Println(sorted)
	fmt.Println(count)
	...
	for len(openSet) != 0 {
		...
		for _, neigh := range neighbors {
			...
			if tentative_score < gScore[neigh] {
				...
				unsolved := countUnsolved(neigh, goal)
