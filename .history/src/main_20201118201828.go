...
func countLinearConflict(board, goal *utils.Grid) int {
	conflict := make(map[[]int]bool,len(board.Values))
	for i, k := range board.Values {
		point := utils.IndexToPoint(i)
		for i2, k2 := range board.Values {
			if k == k2 {
				continue
			}
			point2 := utils.IndexToPoint(i2)
			if point.X == point2.X || point.Y == point2.Y {
				goal1 := utils.IndexToPoint(goal.GetIndex(k))
				goal2 := utils.IndexToPoint(goal.GetIndex(k2))
